...
## Timeline Sync (PluralEyes-style, Sept 2026)

Top-level **Timeline Sync** tab (`#timeline-sync-view`, `switchView('timeline-sync')`): offline multi-camera / external-recorder sync by audio cross-correlation, emitting an FCP7 XML (`xmeml` v5) that Premiere and Resolve import. Validated on real footage (lip sync confirmed in Premiere) — true matches score z≈40–100, non-overlapping files z≈5 (threshold `MIN_CONFIDENCE=8`).

- `src/main/audio-sync-handler.js` — pure Node (no Electron). `analyze(files)`: probes via `ffmpeg -i` stderr (no ffprobe is bundled), stream-decodes each file with `-vn` to mono 8 kHz and reduces to a 200 Hz log-RMS envelope (whitened: 0.5 s high-pass + z-norm), decodes fan out across all cores. Longest audio file = reference; every other file is FFT cross-correlated (own radix-2 FFT, no dependency) against the reference, then against already-placed files if the peak is weak (transitive chaining). Reports per file: `offset` (s, relative to reference, may be negative), `confidence` (z-score), `synced`, `drift` (first-vs-last 5 min local offsets over the overlap; report-only, no correction — measured 10 ppm on a Zoom recorder, i.e. <½ frame per 25 min). `--selfcheck` runs the synthetic-noise assertion.
- `src/main/timeline-xml.js` — `buildXmeml({ name, items })`: sequence fps = first video's fps, one video track per camera, one audio track per channel per file, all clipitems of a file `<link>`ed; `pathurl` is `file://localhost/G:/...` percent-encoded (Hebrew paths verified). Unsynced items are skipped.
- `scripts/sync-test.js [--out x.xml] <files|folders>` — CLI runner; writes `kolbo-synced.xml` next to the first input by default.
- UI: `src/renderer/js/quick-tools/audio-sync-tool.js` (`AudioSyncTool`, tool id `sync`). It is still *registered* through `quick-tools-manager.js` (dropzone/browse/file-path plumbing keyed by `qt-sync-*` element ids) but its container `#qt-sync-content` lives in the separate `#timeline-sync-view` in `index.html`, not under the Quick Tools cards. After every run it draws a lane-per-file timeline preview (`timelineHtml()`, `.qt-sync-*` CSS in `quick-tools.css`); unmatched files appear as dashed grey bars, IPC `qt:audio-sync` in `main.js` `setupQuickToolsHandlers` (progress via `qt:audio-sync-progress`), preload `quickTools.audioSync` / `onAudioSyncProgress`. **Sync only analyzes**; nothing is written until the user reviews the offsets and clicks "Save Timeline XML…", which opens a save dialog (`qt:audio-sync-save`, defaults next to the first file) — Zohar's explicit rule: never auto-write the XML.
- i18n gotcha: English strings come from the **embedded `enFallback` table in `i18n/i18n.js`**, not `en.json` — new keys must be added in both or they render as raw keys.
- Not yet: drift correction, multicam clip output, "Send to Premiere". Split recorder files (REC0004→REC0005) sync independently and land back-to-back; a continuation with no camera overlap is reported unsynced rather than guessed.

## Empty-state illustrations (Kobi, Sept 2026)

Every drop zone / empty state shows a Kobi illustration (`src/renderer/assets/empty-states/*.webp`, 1400px, ~30-70 KB each): `trimmer`, `extractor`, `frame-grabber`, `merger`, `cropper`, `timeline-sync`, `format-factory`, `downloader`, plus `file-bridge` (spare, File Bridge has no empty state). Markup: `<img class="empty-illustration" src="assets/empty-states/<name>.webp">` replaces the old lucide icon in each zone (`index.html`); styling in `desktop-polish.css` (`.empty-illustration`, bottom fade mask, lift on hover/dragover). Generated with Kolbo Creative Director, Nano Banana 2, Visual DNA "Kobi" (`6a54e4e7c56545ad8f90af07`), 16:9 2K, "clean dark-grey stage, no text" brief — URLs + rejected takes in `.kolbo/production.md` (gitignored). To add a tool: generate one scene with the same brief, convert via `sharp` to 1400px WebP q78, drop the `<img>` in.
...
Mac release workflows import the signing certificate once into an explicit runner-temporary keychain and export CSC_KEYCHAIN. Do not also pass CSC_LINK to the build step: that causes a second import and can fail unlocking the builder-created keychain.
